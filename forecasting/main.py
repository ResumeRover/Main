...
# Enable CORS for frontend running on a different origin
app.add_middleware(
    CORSMiddleware,
...
class ARIMAForecastModel:
    ...
    def connect_to_mongodb(self):
        ...
        try:
            ...
            print(f"Connected to MongoDB: {self.db_name}.{self.forecasting_collection}, {self.openings_collection}")
    ...
    def load_data(self, job_role):
        ...
        try:
            if self.forecast_coll is None or self.openings_coll is None:
                print("MongoDB connection not available. Using sample data.")
            ...
            if not job_details:
                print(f"No job opening details found for {job_role} in {self.openings_coll}")
                return None, None
            
            # Fetch historical data
            documents = list(self.forecast_coll.find({"job_role": job_role}))
            if not documents:
                print(f"No historical data found for {job_role}")
            ...
            print(f"Loaded {len(data)} records for {job_role}")
            return data, job_details
        
        except Exception as e:
            print(f"Error loading data for {job_role}: {e}")
            return self._create_sample_data(job_role), None
    
    def _create_sample_data(self, job_role):
        """Create sample data for a job role."""
        print(f"Creating sample data for {job_role}...")
    ...
    def train_models(self, role, job_starting_date, job_closing_date):
        ...
        # role_data = role_data[role_data['date'] >= job_starting_date]
        ...
        try:
            ...
            print(f"Error creating model for {role}: {e}")
    ...
    def calculate_accuracy(self, role):
        ...
        if len(test_data) > 0:
            ...
            try:
                ...
                print(f"Error calculating accuracy for {role}: {e}")
                self.accuracy_metrics[role] = {'mae': 0, 'mape': 0, 'accuracy': 0}
        else:
            self.accuracy_metrics[role] = {'mae': 0, 'mape': 0, 'accuracy': 0}
...
async def get_dashboard_data(job_role: str):
    """API endpoint to return dashboard data for a job role."""
    data, job_details = model.load_data(job_role)
    dashboard_data = model.prepare_dashboard_data(job_role, data, job_details)
    print(f"Dashboard data for {job_role}: {dashboard_data}")
